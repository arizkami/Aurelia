...
fn main() {
    ...
    match bundle_with_bun(&renderer, &package, &bundle) {
        Ok(()) => println!("cargo:rustc-env=SPHEREKIT_MUSICPLAYER_BUNDLE=react"),
        Err(reason) => {
            println!("cargo:warning=musicplayer: {reason}");
            println!(
                "cargo:warning=musicplayer: falling back to the no-bundler renderer; run \
                 `bun install` in app/musicplayer/renderer for the React one"
            );
            fs::write(&bundle, FALLBACK_RENDERER).expect("write fallback renderer");
            println!("cargo:rustc-env=SPHEREKIT_MUSICPLAYER_BUNDLE=fallback");
        }
    }
}
...
///
/// It speaks the API Bridge wire protocol by hand. Useful to read: this is the
/// shape every commit React's reconciler produces, and it is enough to prove
/// the audio path works without a JavaScript toolchain anywhere in the build.
...
(function () {
  ...
  function send(message) {
    return JSON.parse(__spherekitSend(JSON.stringify(message) + "
"));
  }
  ...
  globalThis.__spherekitReceive = function () {};
  ...
  send({
    ...
    snapshot: {
      revision: revision,
      children: [
        node("view", { className: "app" }, [
          node("view", { className: "stage" }, [
            node("view", { className: "now-playing" }, [
              text("Renderer bundle unavailable", "now-title"),
              text("Run `bun install` in app/musicplayer/renderer, then rebuild.", "now-album")
            ]),
            node("view", { className: "viz-row" }, [
              node("spectrum", { className: "spectrum" }, []),
              node("view", { className: "meters" }, [
                node("level-meter", { className: "meter", channel: "left" }, []),
                node("level-meter", { className: "meter", channel: "right" }, [])
              ])
            ]),
            node("waveform", { className: "wave" }, [])
          ])
        ])
      ]
    }
