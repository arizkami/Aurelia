//! Converting a parsed SVG into Sphere geometry.
//!
//! `usvg` does the parsing, the `use`/`defs`/`symbol` resolution and the shape
//! normalisation; everything here turns its output into `sphere-core` types and
...
impl SvgDocument {
    ...
    pub fn fit_transform(&self, dest: Rect<Px>) -> sphere_core::Affine {
        let (vw, vh) = (self.view_box.width.get(), self.view_box.height.get());
        if vw <= 0.0 || vh <= 0.0 || dest.is_empty() {
            return sphere_core::Affine::IDENTITY;
        }
        ...
        sphere_core::Affine::uniform_scale(scale).then(sphere_core::Affine::translate(offset))
    }
}
    ...
    Ok(match paint {
        ...
        usvg::Paint::RadialGradient(g) => {
            let mut gradient = Gradient::Radial {
                ...
                // Sphere's radial gradient is elliptical; SVG's is circular
