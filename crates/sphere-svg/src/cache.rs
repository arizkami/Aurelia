...
use sphere_core::{Affine, Color, GenerationalStore, Px, Rect, SvgError, SvgId};
...
impl SvgCache {
        ...
        for shape in &entry.document.shapes {
            ...
            if let Some(fill) = &shape.fill {
                let brush = match tint {
                    Some(color) => sphere_core::Brush::Solid(color),
                    None => fill.clone(),
                };
    ...
    pub fn render_transformed(
        ...
        let bounds = Rect::new(sphere_core::Point::ZERO, document.view_box);
        let _ = bounds;
        for shape in &document.shapes {
            ...
            let path = shape.path.transformed(transform);
            if let Some(fill) = &shape.fill {
                let brush = tint.map(sphere_core::Brush::Solid).unwrap_or_else(|| fill.clone());
                canvas.fill_path(path.clone(), brush.scale_alpha(shape.opacity));
            }
            if let Some(stroke) = &shape.stroke {
                let brush = tint.map(sphere_core::Brush::Solid).unwrap_or_else(|| stroke.clone());
                let mut style = shape.stroke_style.clone();
                style.width = Px(style.width.get() * transform.approx_scale());
                canvas.stroke_path(path, brush.scale_alpha(shape.opacity), style);
            }
...
mod tests {
    use super::*;
    use sphere_core::{ScaleFactor, Size, px, rect, size};
    use sphere_render::Scene;
    ...
    fn rendering_produces_geometry_scaled_into_the_destination() {
        ...
        let mut s = scene();
        {
            let mut canvas = sphere_render::Canvas::new(&mut s);
            assert!(cache.render(
                id,
                &mut canvas,
    ...
    fn a_zero_size_destination_draws_nothing() {
        ...
        let mut s = scene();
        {
            ...
            assert!(!cache.render(
                id,
                &mut canvas,
                Rect::new(sphere_core::Point::ZERO, Size::new(Px::ZERO, px(10.0))),
                None
            ));
        }
        assert!(s.is_empty());
    ...
    fn a_tint_replaces_every_paint() {
        ...
        assert!(
            s.paints.iter().any(|p| p.brush == sphere_core::Brush::Solid(Color::hex(0x0000FF))),
            "the tint did not replace the document's own fill"
        );
        assert!(
            !s.paints.iter().any(|p| p.brush == sphere_core::Brush::Solid(Color::hex(0x00FF00))),
