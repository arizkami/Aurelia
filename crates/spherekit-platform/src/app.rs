...
//!
//! ```no_run
//! use spherekit_platform::{App, AppContext, AppHandler, WindowAttributes, WindowEvent};
//! use spherekit_core::WindowId;
//!
//! struct Editor;
//!
...
//!         cx.create_window(&WindowAttributes::new("SphereKit")).unwrap();
//!     }
//!
...
use spherekit_core::{PlatformError, WindowId};
...
pub struct AppContext<'a> {
    ...
    /// Platform window id to SphereKit window id.
...
///
/// Backend-independent by construction: every parameter is a SphereKit type, so
...
/// A standalone SphereKit application.
///
/// # Threading
///
...
impl<H: AppHandler> ::winit::application::ApplicationHandler for Driver<H> {
    ...
    fn window_event(
        ...
        for spherekit_event in events.drain(..) {
            let is_redraw = matches!(spherekit_event, WindowEvent::RedrawRequested);
            let is_terminal = spherekit_event.is_terminal();
            self.windows.apply(id, &spherekit_event);
            ...
            }
            {
                ...
                self.handler.window_event(&mut cx, id, spherekit_event);
            }
            if is_redraw {
...
mod tests {
    ...
    ///
    /// ```text
    /// cargo test -p spherekit-platform -- --ignored --test-threads=1
    /// ```
    ///
    ...
    fn a_window_opens_draws_once_and_goes_idle() {
        ...
        impl AppHandler for Once {
            fn resumed(&mut self, cx: &mut AppContext<'_>) {
                let attrs = WindowAttributes::new("spherekit-platform test").with_inner_size(
                    spherekit_core::size(spherekit_core::px(320.0), spherekit_core::px(240.0)),
                );
