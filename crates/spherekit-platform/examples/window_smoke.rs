...
//!
//! Run with `cargo run -p spherekit-platform --example window_smoke`.
//!
...
use spherekit_platform::prelude::*;
use spherekit_platform::{App, AppContext, AppHandler, ElementState, Key, NamedKey, WindowId};
...
impl AppHandler for Smoke {
    fn resumed(&mut self, cx: &mut AppContext<'_>) {
        ...
        self.open_window(cx, "SphereKit platform smoke test");
    }
            ...
            WindowEvent::KeyboardInput { key, state, repeat, modifiers, .. } => {
                ...
                match (&key, state) {
                    ...
                    (Key::Character(c), ElementState::Pressed) if c == "c" => {
                        ...
                        match clipboard.set_text("spherekit-platform smoke test") {
