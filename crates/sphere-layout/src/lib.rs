//! # sphere-layout
//!
//! The retained layout tree of `SphereGraphicEngine`: styles, structure,
//! invalidation, geometry, scrolling and hit testing.
//!
//! ```
//! use sphere_layout::{LayoutEngine, LayoutTree, Style, TaffyLayoutEngine};
//! use sphere_core::{Length, px, relative, size};
//!
...
//! [`NodeId`](sphere_core::NodeId) from `sphere-core`, so a widget can hold a
...
//! # use sphere_core::{px, relative, size};
//! # let mut tree = LayoutTree::new();
//! # let root = tree.insert(Style::row().with_size(size(relative(1.0), relative(1.0))));
