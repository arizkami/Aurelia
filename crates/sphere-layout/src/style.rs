...
//!
//! [`Style`] is Sphere's own type. It is deliberately *not* a re-export of the
...
use sphere_core::{Corners, Edges, Length, Px, Size, px, size};
...
pub enum Display {
    ...
    // vocabulary to Sphere, which is a design decision in its own right and is
...
///
/// Sphere has no CSS `static`: every node is a containing block for its
...
/// directions that Sphere resolves at the container instead.
...
///
/// [`Edges::all`](sphere_core::Edges::all) requires a numeric scalar, and
