...
//! public signature anywhere in the crate, and the only thing the rest of Sphere
...
//!
//! `taffy` can snap a computed tree to whole pixels. Sphere does not use it:
...
use sphere_core::{Edges, LayoutError, Length, NodeId, Point, Px, Rect, Size, px, size};
...
/// One mirrored node.
struct MirrorNode {
    /// The Sphere identity this mirrors.
    id: NodeId,
    /// Translated style; rebuilt only when the Sphere style changed.
    style: taffy::Style,
...
impl taffy::LayoutPartialTree for Pass<'_> {
    ...
    /// Named grid lines and areas are not exposed by Sphere's [`Style`], so this
...
/// Sphere `Edges` are `top/right/bottom/left`; `taffy` rects are
...
/// Translates one Sphere style into the backend's representation.
fn to_taffy_style(s: &Style) -> taffy::Style {
    taffy::Style {
        ...
        // Sphere sizes the border box, always. Content-box sizing makes "this
        ...
        // Sphere draws overlay scrollbars, so no space is reserved in the layout.
        scrollbar_width: 0.0,
