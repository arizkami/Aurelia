...
//!   [`Path`](sphere_core::Path) so a tessellator can turn it back into
//!   triangles is work with no product, so
//!   [`Canvas::draw_mesh`](sphere_render::Canvas::draw_mesh) exists and this is
//!   what uses it.
//!
//! ```ignore
...
use sphere_layout::Style;
use sphere_render::{Canvas, Mesh, MeshVertex};
use sphere_ui::{
...
impl RealtimeFrame<'_, '_> {
    ...
    pub fn draw_spectrum(&mut self, magnitudes: &[f32], nyquist_hz: f32, style: &SpectrumStyle) {
        ...
        if style.line_width > Px::ZERO {
            let mut path = sphere_core::PathBuilder::new();
...
mod tests {
    ...
    use sphere_render::{DrawCommand, Scene};
    use sphere_ui::{IntoElement, ParentElement, UiTree, div};
    ...
    fn render(root: sphere_ui::AnyElement) -> (Scene, UiTree) {
    ...
    fn realtime_nodes_scrolled_out_of_a_clip_do_no_work() {
        ...
        let mut row =
            div().flex_row().w(px(STRIPS as f32 * STRIP_WIDTH)).h(sphere_core::relative(1.0));
        for i in 0..STRIPS {
