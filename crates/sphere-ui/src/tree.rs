...
use sphere_core::{ElementId, NodeId, Point, Px, Rect, Size};
use sphere_layout::{DirtyFlags, LayoutEngine, LayoutTree, Style, TaffyLayoutEngine};
...
impl UiTree {
    ...
    pub fn build(&mut self, root: AnyElement) {
        ...
        let root_identity = ElementId::from_key("sphere.root");
        ...
        while let Some(mut pending) = stack.pop() {
            ...
            let clips = !matches!(style.overflow_x, sphere_layout::Overflow::Visible)
                || !matches!(style.overflow_y, sphere_layout::Overflow::Visible);
    ...
    pub fn compute_layout(&mut self, viewport: Size<Px>) -> Result<(), sphere_core::LayoutError> {
    ...
    ) -> Result<(), sphere_core::LayoutError> {
...
mod tests {
    ...
    use sphere_core::{Color, ScaleFactor, px, relative, size};
    ...
    fn a_clipping_container_narrows_the_visible_region_for_its_children() {
        ...
        build_and_layout(
            ...
            div()
                .w(px(50.0))
                .h(px(50.0))
                .overflow(sphere_layout::Overflow::Hidden)
