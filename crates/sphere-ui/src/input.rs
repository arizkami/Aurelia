...
//!
//! `sphere-platform` reports what the window system said. `sphere-ui` needs
...
impl InputTranslator {
    ...
    pub fn translate(&mut self, event: &sphere_platform::WindowEvent) -> SmallVec<[UiEvent; 2]> {
        use sphere_platform::WindowEvent as P;
        ...
        match event {
            ...
            P::MouseWheel { delta, phase } => {
                out.push(UiEvent::Scroll(ScrollEvent {
                    ...
                    modifiers: self.modifiers,
                    momentum: matches!(phase, sphere_platform::TouchPhase::Moved),
                }));
            }
...
fn convert_button(button: sphere_platform::MouseButton) -> MouseButton {
    use sphere_platform::MouseButton as P;
    match button {
...
fn convert_state(state: sphere_platform::ElementState) -> ElementState {
    match state {
        sphere_platform::ElementState::Pressed => ElementState::Pressed,
        sphere_platform::ElementState::Released => ElementState::Released,
    }
}

fn convert_modifiers(modifiers: sphere_platform::Modifiers) -> Modifiers {
    use sphere_platform::Modifiers as P;
    Modifiers {
...
fn convert_scroll(delta: sphere_platform::ScrollDelta) -> ScrollDelta {
    use sphere_platform::ScrollDelta as P;
    match delta {
...
fn convert_key(key: &sphere_platform::Key) -> Key {
    use sphere_platform::{Key as P, NamedKey as N};
    match key {
        ...
        // `sphere_platform::Key` is `#[non_exhaustive]`, so a new variant added
...
fn named_key_code(named: &sphere_platform::NamedKey) -> u32 {
...
fn convert_ime(ime: &sphere_platform::ImeEvent) -> Option<ImeEvent> {
    use sphere_platform::ImeEvent as P;
    Some(match ime {
...
mod tests {
    ...
    fn character_keys_carry_their_text() {
        use sphere_platform::{Key as P, KeyText};
    ...
    fn events_the_ui_does_not_care_about_translate_to_nothing() {
        ...
        assert!(
            t.translate(&P::Resized(Size::new(
                sphere_core::DevicePx(800),
                sphere_core::DevicePx(600)
            )))
            .is_empty()
        );
    }
