...
//! [`TextSystem::layout`](spherekit_text::TextSystem::layout), which is backed by
...
use spherekit_core::{Brush, Color, ElementId, Px, Size};
use spherekit_layout::{AvailableSpace, MeasureRequest, Style};
use spherekit_render::{GlyphRun, PositionedGlyph, TextRasterMode};
use spherekit_text::{TextAlign, TextStyle, TextSystem, WrapMode};
...
impl Label {
    ...
    pub fn italic(mut self) -> Self {
        self.text_style.font.style = spherekit_text::FontStyle::Italic;
        self
    }
    ...
    pub fn truncate(mut self) -> Self {
        self.text_style.wrap = WrapMode::None;
        self.text_style.overflow = spherekit_text::Overflow::Ellipsis;
        self
    }
    ...
    /// against. See [`spherekit_render::GlyphRun::coverage_contrast`].
...
impl Element for Label {
    ...
    fn paint(&mut self, cx: &mut PaintContext<'_, '_>) {
        ...
        let coverage_contrast = self.coverage_contrast.unwrap_or_else(|| {
            spherekit_render::coverage_contrast_for(color, cx.theme.colors.background)
        });
...
pub fn draw_layout(
    canvas: &mut spherekit_render::Canvas<'_>,
    layout: &spherekit_text::TextLayout,
    origin: spherekit_core::Point<Px>,
    color: Color,
    raster: TextRasterMode,
    outline: (Px, Color),
        ...
        for run in &line.runs {
            ...
            let glyphs: SmallVec<[PositionedGlyph; 8]> = run
                ...
                .map(|g| PositionedGlyph {
                    glyph: g.glyph,
                    position: spherekit_core::Point::new(
...
mod tests {
    ...
    use spherekit_core::{ScaleFactor, px, relative, size};
    use spherekit_render::{Canvas, DrawCommand, Scene};
    ...
    fn a_label_inherits_theme_weight_and_an_explicit_weight_wins() {
        ...
        assert_eq!(
            ...
            spherekit_text::FontWeight::BOLD
        );
        assert_eq!(
            label("Override")
                .weight(spherekit_text::FontWeight::LIGHT)
                .resolved_text_style(&theme)
                .font
                .weight,
            spherekit_text::FontWeight::LIGHT
        );
    }
    ...
    fn label_weight_reaches_the_painted_glyph_run() {
        ...
        tree.build(
            div()
                .child(label("Regular").weight(spherekit_text::FontWeight::NORMAL))
                .child(label("SemiBold").weight(spherekit_text::FontWeight::SEMI_BOLD))
                .into_element(),
        );
