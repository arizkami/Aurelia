...
//! they are all used at once. A [`SphereKitSurface`] owns the GPU backend, the UI
...
//! drive [`spherekit_ui::UiTree`] and [`spherekit_wgpu::WgpuRenderer`] directly; this
//! type exists so that the common case is short.

use rustc_hash::FxHashMap;
use spherekit_core::{
    Color, DevicePx, ImageId, InitError, RenderError, ScaleFactor, Size, TextureId,
};
use spherekit_image::ImageCache;
...
pub struct SurfaceStats {
    ...
    pub tree: spherekit_ui::TreeStats,
...
/// One window's worth of SphereKit.
pub struct SphereKitSurface {
...
impl SphereKitSurface {
    ...
    where
        W: spherekit_wgpu::wgpu::WasmNotSendSync
    ...
    ///
    /// Valid after [`SphereKitSurface::render`], because the caret's position is a
    /// paint-time fact. Apply it with
    /// [`spherekit_platform::Window::set_ime_allowed`] and
    /// [`spherekit_platform::Window::set_ime_cursor_area`]; `None` means nothing
    /// focused accepts text and the input method should be switched off.
    #[inline]
    pub fn ime(&self) -> Option<spherekit_ui::ImeArea> {
        self.tree.ime()
    }
    ...
    pub fn load_image(&mut self, bytes: &[u8]) -> Result<ImageId, spherekit_core::ImageError> {
    ...
    pub fn load_rgba8(
        ...
        height: u32,
        rgba: &[u8],
        alpha: spherekit_image::AlphaMode,
    ) -> Result<ImageId, spherekit_core::ImageError> {
    ...
    fn upload_atlas(&mut self) -> Result<u64, RenderError> {
        ...
        let regions: Vec<(u32, GlyphFormat, spherekit_core::Rect<u32>, u32, Vec<u8>)> = self
            .text
            .take_dirty_regions()
            .into_iter()
    ...
    /// Uploads images queued by [`SphereKitSurface::load_image`].
