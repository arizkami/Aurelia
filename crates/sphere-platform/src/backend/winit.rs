...
//! functions are `pub(crate)`, and [`Window`] exposes only Sphere types plus
...
///
/// The value is kept even when Sphere has no name for the key: a control
...
pub(crate) fn physical_key_from_winit(key: ::winit::keyboard::PhysicalKey) -> PhysicalKey {
    match key {
        ::winit::keyboard::PhysicalKey::Code(c) => match key_code_from_winit(c) {
            ...
            // A position winit names but Sphere does not. There is no raw value
...
/// Translates one platform event into zero, one or two Sphere events.
///
...
pub(crate) fn translate_window_event(
    ...
    match event {
        ...
        // have no Sphere equivalent yet. Dropping them is deliberate: a
...
/// Maps a Sphere cursor to the platform icon, or `None` for
...
/// Maps Sphere's control flow onto the platform loop's.
///
...
mod tests {
    ...
    fn named_keys_translate_by_name() {
        ...
        // A key Sphere does not name is dropped, not mistranslated.
