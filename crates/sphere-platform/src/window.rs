...
//! created the process, runs the event loop (or a native one Sphere never
...
//!
//! * [`WindowTarget::Owned`] wraps a window Sphere created and will destroy.
...
//! 2. **Sphere never resizes a foreign window.** The host owns its geometry.
...
/// A handle to a window Sphere does not own.
///
...
impl ForeignWindow {
    ...
    /// Records a resize reported by the host. Sphere never initiates one.
    #[inline]
...
///
/// This is the seam between `sphere-platform` and `sphere-wgpu`. Both variants
...
pub enum WindowTarget {
    /// A window Sphere created and owns.
...
pub struct WindowAttributes {
    ...
    /// Sphere creates a real child window inside it, which keeps input routing
