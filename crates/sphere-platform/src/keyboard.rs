...
//!
//! Sphere separates the three things a keystroke actually carries, because
...
pub enum Key {
    ...
    Dead(Option<char>),
    /// The platform reported a key Sphere has no name for. The physical
...
/// A raw platform key identifier, kept for keys Sphere cannot name.
///
...
impl Scancode {
    ...
    ///
    /// Zero is used as the sentinel because no platform Sphere targets reports
...
pub enum PhysicalKey {
    /// A position Sphere has a name for.
    Code(KeyCode),
    /// A position Sphere cannot name, carrying the raw platform value so that
