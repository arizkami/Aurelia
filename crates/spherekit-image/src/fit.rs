...
//! becomes numbers. `spherekit-render` defers to it rather than reimplementing the
...
use spherekit_core::{ImageFit, Point, Px, Rect, Size};
...
mod tests {
    use super::*;
    use spherekit_core::{px, rect, size};
    ...
    fn source_uv_normalises_the_crop() {
        ...
        let uv = r.source_uv(natural);
        assert_eq!(uv, spherekit_core::rect(0.25, 0.0, 0.5, 1.0));
    }
