...
use spherekit_core::{ImageError, Px, Size, px};
...
/// and matched by every desktop backend SphereKit targets. Hosts that queried a
...
///
/// SphereKit never silently linearises image data. A PNG or JPEG is sRGB-encoded,
...
/// A container format `spherekit-image` can decode.
///
...
impl ImageFormatKind {
    ...
    /// from "a real image format that SphereKit does not decode".
    ...
    let kind = match ImageFormatKind::classify(bytes) {
        ...
        MagicVerdict::Recognised(name) => {
            return Err(ImageError::UnsupportedFormat(format!(
                "{name} (spherekit-image decodes PNG, JPEG and WebP)"
            )));
        }
...
pub(crate) mod tests {
    ...
    fn format_is_detected_from_content_not_extension() {
        ...
        let dir = std::env::temp_dir().join("spherekit-image-tests");
    ...
    fn missing_file_is_an_io_error_carrying_the_path() {
        let path = Path::new("W:/definitely/not/here/spherekit-image-missing.png");
        match decode_path(path, &DecodeOptions::default()) {
            Err(ImageError::Io { path: p, .. }) => assert!(p.contains("spherekit-image-missing")),
