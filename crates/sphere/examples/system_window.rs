...
//!
//! ```bash
//! cargo run -p sphere --example system_window --release
//! SPHERE_DEMO_FRAMES=120 cargo run -p sphere --example system_window --release
//! ```
...
use sphere::core::{Px, px, relative, size};
use sphere::platform::{
...
impl AppHandler for SystemWindowApp {
    fn resumed(&mut self, cx: &mut AppContext<'_>) {
        ...
        let attrs = WindowAttributes::new("SphereGraphicEngine — Project settings")
