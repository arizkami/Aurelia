...
//!
//! ```text
//! cargo run -p sphere --example plugin_ui_demo --release
//! ```
//!
...
use sphere::core::{Color, Corners, DevicePx, Px, RoundedRect, Size, px, rect, relative, size};
use sphere::platform::{
    App, AppContext, AppHandler, RedrawPolicy, WindowAttributes, WindowEvent, WindowId,
};
use sphere::render::{Canvas, TextRasterMode};
use sphere::ui::{
...
/// reads. Sharing an `Rc` into the callback is the pattern Sphere intends:
...
struct Demo {
    ...
    /// Frames drawn so far.
    frames: u64,
    /// Exit after this many frames, from `SPHERE_DEMO_FRAMES`.
    ///
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .child(
                        realtime_canvas(move |frame| {
                            let curve = sphere::audio::CompressorCurve {
...
/// accessibility reporting all come from `sphere_ui::knob`; this only supplies
...
impl AppHandler for Demo {
    fn resumed(&mut self, cx: &mut AppContext<'_>) {
        ...
        let surface = pollster::block_on(SphereSurface::new(
            Arc::clone(&window),
        ...
        for ui_event in ui_events {
            let (result, hit_bypass) = {
                ...
                let hit = surface
                    ...
                    .focus()
                    .focused()
                    .is_some_and(|f| f == sphere::core::ElementId::from_key("bypass"));
            ...
            if !result.consumed
                && let sphere::ui::UiEvent::Key(key) = &ui_event
                && key.state.is_pressed()
            {
                match &key.key {
                    sphere::ui::Key::Character(c) if c.eq_ignore_ascii_case("b") => {
                        self.bypassed.set(!self.bypassed.get());
                    }
                    sphere::ui::Key::Character(c) if c.eq_ignore_ascii_case("s") => {
                        self.show_stress = !self.show_stress;
                    }
                    sphere::ui::Key::Character(c) if c.eq_ignore_ascii_case("t") => {
