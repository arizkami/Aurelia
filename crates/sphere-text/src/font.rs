...
//! without a self-referential struct. Sphere solves this the boring way: the
...
use sphere_core::{FontError, FontId, GenerationalStore, GlyphId};
...
fn metrics_of(face: &ttf_parser::Face<'_>) -> FontMetrics {
    ...
    // `descender` is negative in font units; Sphere stores descent as a
    ...
    let (underline_offset, underline_thickness) = match face.underline_metrics() {
        ...
        // negative; Sphere wants a downward offset.
...
/// Maps Sphere's width class onto `fontdb`'s.
...
/// Maps Sphere's slant onto `fontdb`'s.
