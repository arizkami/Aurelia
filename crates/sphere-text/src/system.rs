...
//! implements [`sphere_render::GlyphProvider`] so the batch compiler can ask it
    ...
    fn rasterize(
        ...
        if image.is_empty() {
            return Some(crate::types::AtlasPlacement {
                ...
                texels: sphere_core::Rect {
                    origin: sphere_core::Point { x: 0, y: 0 },
                    size: sphere_core::Size { width: 0, height: 0 },
                },
        ...
        match self.atlas.insert(key, &image) {
            ...
            Err(e) => {
                ...
                // window is not.
                tracing::warn!(target: "sphere_text", "glyph atlas insert failed: {e}");
